...
public class SmsJourneyCache {
    ...
    public SmsJourneyCache(SmsJourneyRepository smsJourneyRepository, @Value("${sms-journey.cache.expire-after-write}") Duration expireAfterWrite) {
        ...
        this.cache = Caffeine.newBuilder().maximumSize(1000).expireAfterWrite(expireAfterWrite).build(smsId -> {
            ...
            return smsJourneyRepository.findBySmsSmsId(smsId).stream().map(this::convertToDto).toList();
        });
    }

    public List<SmsJourneyDto> getJourneysBySmsId(Long smsId) {
